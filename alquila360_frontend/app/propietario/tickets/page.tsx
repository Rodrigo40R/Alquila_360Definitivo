...
type TicketBackend = {
  ...
  // Relación anidada: NECESITAMOS el ID del propietario en la propiedad
  propiedad?: {
    direccion: string;
    id_propietario: number; // Campo crucial para el filtrado en el front
  };
};
...
export default function PropietarioMantenimientoPage() {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      ...
      const userId = user.id; // ID limpio del propietario

      try {
        const baseUrl = process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001";
        
        // 🚨 CAMBIO: Llamamos al endpoint general /tickets (para evitar el 404)
        const res = await fetch(`${baseUrl}/tickets`, {
          headers: { "Authorization": `Bearer ${user.token}` }
        });

        if (res.ok) {
          const allTickets: TicketBackend[] = await res.json();
          
          // 🚨 FILTRADO EN EL FRONTEND: Filtramos solo los tickets de las propiedades que pertenecen a este userId
          const filteredTickets = allTickets.filter(t => 
              t.propiedad?.id_propietario === userId
          );
  ...
  // 3. FILTRADO DE LISTA (por pestaña activa)
